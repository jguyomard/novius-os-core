...
class Controller_Admin_Upload extends \Nos\Controller_Admin_Application
{
    ...
    public function post_save()
    {
        ...
        if ($this->nbDuplicateError > 0) {
            $error = n__('One file already exist!', '{{count}} files already exist!', $this->nbDuplicateError);
            $error = strtr($error, array('{{count}}' => $this->nbDuplicateError));
        }
        \Response::json(array(
            'notify' => __('Done! All files have been uploaded.').' '.$error,
    ...
    protected function _importFiles($files, $path, $folder)
    {
        ...
        foreach ($files as $dir => $file) {
            try {
                ...
                \Log::error('Upload : '.$file.' already exists');
                $this->nbDuplicateError++;
            }
        }
    }
